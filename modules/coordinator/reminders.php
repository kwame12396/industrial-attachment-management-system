<?php
// modules/coordinator/reminders.php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $title = $conn->real_escape_string($_POST['title'] ?? 'Reminder from Coordinator');
    
    // Get users based on role
    if ($recipient_type == 'all') {
        $users = $conn->query("SELECT id FROM users WHERE role IN ('student', 'industrial_supervisor', 'university_supervisor')");
    } else {
        $users = $conn->query("SELECT id FROM users WHERE role = '$recipient_type'");
    }
    
    $count = 0;
    while($user = $users->fetch_assoc()) {
        createNotification($user['id'], $title, $message, 'reminder', $due_date);
        $count++;
    }
    
    $_SESSION['success'] = "Reminder sent to $count users!";
    redirect('modules/coordinator/reminders.php');
}

// Get statistics for display
$students = $conn->query("SELECT COUNT(*) as count FROM users WHERE role='student'")->fetch_assoc()['count'];
$industrial = $conn->query("SELECT COUNT(*) as count FROM users WHERE role='industrial_supervisor'")->fetch_assoc()['count'];
$university = $conn->query("SELECT COUNT(*) as count FROM users WHERE role='university_supervisor'")->fetch_assoc()['count'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Send Reminders - IAMS</title>
    ...
    <style>
        ...
        .btn-send { background: linear-gradient(135deg, #4361ee, #3f37c9); color: white; border: none; border-radius: 10px; padding: 10px 20px; }
    </style>
</head>
<body>
    <div class="sidebar">
    ...
    <div class="content">
        <h3><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="display:inline-block;vertical-align:middle;flex-shrink:0;margin-right:0.5rem;"><path fill="none" stroke-linecap="round" stroke-linejoin="round" d="M6 13.25h2m3-7.5a4 4 0 0 0-8 0v3.5a1.5 1.5 0 0 1-1.5 1.5h11a1.5 1.5 0 0 1-1.5-1.5ZM.5 5.62A6 6 0 0 1 3 .75m10.5 4.87A6 6 0 0 0 11 .75"/></svg>Send Reminders</h3>
        <?php displayMessage(); ?>
        
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header bg-white"><h5>Compose Reminder</h5></div>
                    <div class="card-body">
                        <form method="POST">
                            <div class="mb-3">
                                <label>Reminder Title</label>
                                <input type="text" name="title" class="form-control" value="Reminder from Coordinator" required>
                            </div>
                            <div class="mb-3">
                                <label>Recipients</label>
                                <select name="recipient_type" class="form-control" required>
                                    <option value="student">All Students (<?php echo $students; ?>)</option>
                                    <option value="industrial_supervisor">All Industrial Supervisors (<?php echo $industrial; ?>)</option>
                                    <option value="university_supervisor">All University Supervisors (<?php echo $university; ?>)</option>
                                    <option value="all">All Users (<?php echo $students + $industrial + $university; ?>)</option>
                                </select>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="message" class="form-control" rows="4" required placeholder="Enter reminder message..."></textarea>
                            </div>
                            <button type="submit" class="btn btn-send w-100"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="display:inline-block;vertical-align:middle;flex-shrink:0;margin-right:0.5rem;"><g fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M7 13.5a6.5 6.5 0 1 0 0-13a6.5 6.5 0 0 0 0 13M5.5 10h3"/><path d="M7 10V6.5H6m1-2.25a.25.25 0 0 1 0-.5m0 .5a.25.25 0 0 0 0-.5"/></g></svg>Send Reminder</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header bg-white"><h5>Quick Tips</h5></div>
                    <div class="card-body">
                        <ul>
                            <li>Reminders appear in the user's notification panel on their dashboard.</li>
                            <li>Students will see a red badge on the notifications icon.</li>
                            <li>You can set an optional due date for deadlines.</li>
                            <li>Reminders are stored in the database and can be marked as read by recipients.</li>
                        </ul>
                    </div>
                </div>
            </div>
