...
$sup_id = $sup['id'];
$reports = $conn->query("SELECT ir.*, s.full_name, s.student_id AS sid, s.program FROM industrial_reports ir JOIN students s ON ir.student_id=s.id WHERE ir.supervisor_id=$sup_id ORDER BY ir.submitted_at DESC");
?><!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
    <title>My Reports - IAMS</title>
    ...
    <style>
        ...
        .rec-excellent{color:#065f46;background:#d1fae5;} .rec-good{color:#1e40af;background:#dbeafe;} .rec-average{color:#92400e;background:#fef3c7;} .rec-poor{color:#991b1b;background:#fee2e2;}
    </style>
</head>
<body>
<div class="sidebar">
...
<div class="content">
    <h4 class="mb-4"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="display:inline-block;vertical-align:middle;flex-shrink:0;margin-right:0.5rem;color:var(--bs-success);"><path fill="none" stroke-linecap="round" stroke-linejoin="round" d="m1.24 6.54l11.5-5.23M10.59.5l2.15.81l-.8 2.15m1.31 10.05h-2.5h0v-7a.5.5 0 0 1 .5-.5h1.5a.5.5 0 0 1 .5.5v7h0Zm-5 0h-2.5h0v-5.5a.5.5 0 0 1 .5-.5h1.5a.5.5 0 0 1 .5.5v5.5h0Zm-5 0H.75h0v-4a.5.5 0 0 1 .5-.5h1.5a.5.5 0 0 1 .5.5v4h0Z"/></svg>Submitted Performance Reports</h4>
    <?php displayMessage(); ?>
    <div class="card">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead><tr class="table-light"><th class="ps-4">Student</th><th>Programme</th><th>Overall</th><th>Attendance</th><th>Technical</th><th>Communication</th><th>Teamwork</th><th>Recommendation</th><th>Submitted</th><th>Action</th></tr></thead>
                    <tbody>
                    <?php while($r=$reports->fetch_assoc()): ?>
                    <tr>
                        <td class="ps-4"><strong><?php echo htmlspecialchars($r['full_name']); ?></strong><div style="font-size:0.78rem;color:#555;"><?php echo htmlspecialchars($r['sid']); ?></div></td>
                        <td style="font-size:0.85rem;"><?php echo htmlspecialchars($r['program']); ?></td>
                        <td><strong><?php echo $r['overall_rating']; ?>/10</strong></td>
                        <td><?php echo $r['attendance_rating']; ?>/10</td>
                        <td><?php echo $r['technical_skills_rating']; ?>/10</td>
                        <td><?php echo $r['communication_rating']; ?>/10</td>
                        <td><?php echo $r['teamwork_rating']; ?>/10</td>
                        <td><span class="badge rec-<?php echo $r['recommendation']; ?> px-2 py-1 rounded-pill"><?php echo ucfirst($r['recommendation']); ?></span></td>
                        <td style="font-size:0.82rem;"><?php echo date('d M Y', strtotime($r['submitted_at'])); ?></td>
                        <td><a href="submit_report.php?student_id=<?php echo $r['student_id']; ?>" class="btn btn-sm btn-outline-primary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="display:inline-block;vertical-align:middle;flex-shrink:0;"><g fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="m7.5 9l-3 .54L5 6.5L10.73.79a1 1 0 0 1 1.42 0l1.06 1.06a1 1 0 0 1 0 1.42Z"/><path d="M12 9.5v3a1 1 0 0 1-1 1H1.5a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1h3"/></g></svg></a></td>
                    </tr>
                    <?php endwhile; ?>
                    </tbody>
                </table>
